...
class TB():
  ...
  def __init__(self, dut, nTaps: int, cutoffFreq: int | list[int], lowpass: bool, fs: int, precBits=15):
    ...
    dut.s_axis_dl_tdata.value = Immediate(0)
    dut.s_axis_dl_tvalid.value = Immediate(0)

    dut.wren.value = Immediate(0)
    dut.w_coef_data.value = Immediate(0)
    dut.w_coef_addr.value = Immediate(0)
    dut.filter_depth.value = Immediate(0)

    dut.m_axis_mac_tready.value = Immediate(0)
  ...
  async def load_IR(self, ir=None):
    self.dut.m_axis_mac_tready.value = 1
    if ir is None:
      ir = self.filt.fixedCoeffs

    await RisingEdge(self.dut.clk)
    self.dut.filter_depth.value = len(ir)
    self.dut.wren.value = 1
    addr = 0
    for el in ir:
      self.dut.w_coef_data.value = int(el)
      self.dut.w_coef_addr.value = addr
      addr += 1
      await RisingEdge(self.dut.clk)
    self.dut.w_coef_addr.value = 0
    self.dut.wren.value = 0


  async def pass_signal(self, signal):
    await RisingEdge(self.dut.clk)
    self.dut.s_axis_dl_tvalid.value = 1 
    for el in signal:
      self.dut.s_axis_dl_tdata.value = (el << 16)
      await RisingEdge(self.dut.s_axis_dl_tready) 
      await RisingEdge(self.dut.clk)
    self.dut.s_axis_dl_tvalid.value = 0
  ...
  async def passComplexSignal(self, signal):
    ...
    for el in signal:
      ...
      self.dut.s_axis_dl_tdata.value = str(r) + str(im)
      await RisingEdge(self.dut.s_axis_dl_tready) 
      await RisingEdge(self.dut.clk)
    self.dut.s_axis_dl_tvalid.value = 0
  ...
  async def mon_output(self, event: Event):
    result = []
    resValid = RisingEdge(self.dut.m_axis_mac_tvalid)
    ev = event.wait()
    while True:
      ...
      if trig is ev:
        ...
        await Timer(1, "ns")
        result.append(self.dut.m_axis_mac_tdata.value)
    self.logger.debug("Mon Stop")
    self.dut.m_axis_mac_tready.value = 0
    return result
...
async def test_IR(dut):
  ...
  tb.dut.m_axis_mac_tready.value = 1
  await tb.init()
  await tb.load_IR()
  await Timer(100, "ns")
  ev = Event()
  ...
  tb.dut.m_axis_mac_tready.value = 0
  ev.set()
  await Timer(100, "ns")
