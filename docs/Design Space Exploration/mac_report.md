...
После выполнения имплементации на оптимизированном варианте был проведен дополнительный временной анализ с целью выявить критические пути, которые замедляют ограничивают максимальную работу схемы.

![](./img/dts.png)

Выжимка временного анализа показала непрохождение ограничений по `Setup` и `Pulse Width`. Общее количество непроходящих путей - 1120, сделаем отдельный репорт на эти непроходящие пути.

### Pulse Width

![](./img/pulse_width.png) 

На картинке показаны непроходящие пути. Наибольшим недопуском обладает минимальный период тактовой частоты для блоков `RAMB18E1`. Временными ограничениями был создан клок частотой периодом 2 нс, что оказалось слишком маленьким периодом для корректной работы блоков памяти на кристалле ПЛИС

Максимальная частота работы ПЛИС семейства Artix7 ограничена максимальной частотой работы блоков памяти.

В итоге была понижена частота работы до 333 МГц.

### Setup

![](./img/setup.png)

На картинке показаны непроходящие пути по Setup, конкретно пути на картинке - это пути от управляющего автомата до входа адреса блока памяти. Проблемы по Setup связаны с тем, что данные не успевают выставиться и продержаться необходимое время перед захватом данных триггером перед фронтом сигнала. Данные задерживаются из-за цепочки комбинационной логики, разделить цепочку комбинационной логики можно регистрами в пути сигнала.
